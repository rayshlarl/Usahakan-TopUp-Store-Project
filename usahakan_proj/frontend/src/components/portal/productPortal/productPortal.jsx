...
export const FormProductPortal = ({
  onClose,
  onSubmit,
  categorys,
  inputTypes,
  ...
  const [formData, setFormData] = useState({
    inputStyle: defaultValue?.input_types?.name || "",
    category: defaultValue?.category_name || "",
    name: defaultValue?.name || "",
    desc: defaultValue?.description || "",
    isAvailable: defaultValue?.is_available || true,
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newErrors = {};
    console.log(getUploadUrl("thumbnails", `${defaultValue?.image}`));
    // --> Validation Logic
              ...
              <div className="w-32 h-50 bg-gray-100 rounded-xl border-2 border-dashed border-gray-300 flex items-center justify-center">
                <img
                  src={
                    previewUrl
                      ? previewUrl
                      : getUploadUrl("thumbnails", `${defaultValue?.image}`)
                  }
                  alt=""
                />
              </div>
